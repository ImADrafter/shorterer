...
function postController(req, res) {
    ...
    Urls.findOne({ url }, function (error, urlFound) {
        ...
        if (urlFound) {
            res.send(`The requested url ${formatRawText(urlFound, 'red')} is already in use in the database ${backButton}`)
        } else {
            increaseCount(req, res, function (count) {
                const newReqObj = { ...urlObj, short_url: count }
                const newEntry = new Urls(newReqObj)
                    .save(function (error) {
                        if (error) { return res.send(`Couldn't add the url [${formatRawText(newReqObj, 'red')}] ${backButton}`) };
                        res.send(`Successfully added the url ${formatRawText(newReqObj, 'blue')}`)
                    })
            })
        }
    })
